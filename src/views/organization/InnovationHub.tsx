...
export const InnovationHub: React.FC = () => {
  ...
  const handleScoreIdea = async () => {
    ...
    setIsScoring(true);
    try {
      const result = await api.organization.scoreIdea({ title: ideaTitle, description: ideaDesc });
      setScoreResult({
  ...
  return (
    <div className="flex flex-col gap-8 relative pb-4" style={{ paddingBottom: '3rem' }}>
      ...
      {reviewIdeaId !== null && (() => {
        ...
        if (!idea) return null;
        return (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 transition-opacity duration-300" style={{ backgroundColor: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(24px)' }}>
            <div className="bg-white/70 backdrop-blur-3xl rounded-[2.5rem] max-w-4xl w-full shadow-[0_30px_80px_-15px_rgba(0,0,0,0.5)] relative overflow-hidden animate-in zoom-in-95 duration-300 border border-white/80 flex flex-col max-h-[90vh]">
              {/* Decorative Glowing Orbs behind the content */}
              <div className="absolute -top-40 -right-40 w-96 h-96 bg-blue-400/20 rounded-full blur-[80px] pointer-events-none"></div>
              <div className="absolute -bottom-40 -left-40 w-96 h-96 bg-emerald-400/20 rounded-full blur-[80px] pointer-events-none"></div>

              {/* Premium Light Header */}
              <div className="relative px-8 sm:px-12 pt-12 pb-10 shrink-0 bg-white/40 border-b border-white/50 backdrop-blur-md">
                <button
                  onClick={() => setReviewIdeaId(null)}
                  className="absolute top-6 right-6 w-10 h-10 flex items-center justify-center rounded-full bg-slate-50/50 hover:bg-slate-100/80 backdrop-blur-sm transition-colors text-slate-500 hover:text-slate-700 z-20 cursor-pointer"
                >
                  <X size={20} />
                </button>

                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-[1.25rem] flex items-center justify-center text-white font-extrabold text-2xl sm:text-3xl shrink-0 shadow-[0_8px_20px_rgba(0,0,0,0.15)]" style={{ background: getAuthorGradient(idea.authorBg) }}>
                    {idea.authorInitials}
                  </div>
                  <div className="flex-1 pr-8 w-full">
                    <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900 mb-4 tracking-tight leading-tight">{idea.title}</h2>
                    <div className="flex flex-wrap gap-2.5">
                      <span className="px-3.5 py-1.5 rounded-xl text-xs font-bold text-emerald-700 bg-emerald-100/50 border border-emerald-300/50 flex items-center gap-1.5 shadow-[0_0_15px_rgba(16,185,129,0.15)]">
                        <TrendingUp size={14} className="text-emerald-500" /> Impact: {idea.impactScore}/100
                      </span>
                      <span className="px-3.5 py-1.5 rounded-xl text-xs font-bold text-blue-700 bg-blue-100/50 border border-blue-300/50 flex items-center gap-1.5 shadow-[0_0_15px_rgba(59,130,246,0.15)]">
                        <BrainCircuit size={14} className="text-blue-500" /> Feasibility: {idea.feasibility}
                      </span>
                      <span className="px-3.5 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-sm backdrop-blur-md" style={{
                        backgroundColor: idea.status === 'Approved' ? 'rgba(236, 253, 245, 0.7)' : 'rgba(248, 250, 252, 0.7)',
                        color: idea.status === 'Approved' ? '#047857' : '#475569',
                        border: `1px solid ${idea.status === 'Approved' ? 'rgba(110, 231, 183, 0.6)' : 'rgba(203, 213, 225, 0.6)'}`
                      }}>
                        {idea.status}
                      </span>
                    </div>
              ...
              {/* Scrollable Body */}
              <div className="px-8 sm:px-12 py-10 overflow-y-auto flex-1 relative bg-transparent">
                <div className="space-y-10 relative z-10">

                  {/* Summary */}
                  <div className="w-full">
                    <h3 className="text-[11px] font-extrabold text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                      <FileText size={16} className="text-indigo-500" /> Executive Summary
                    </h3>
                    <div className="p-6 sm:p-8 rounded-[1.5rem] bg-white/60 border border-white/80 backdrop-blur-xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] leading-relaxed text-slate-700 font-medium text-[15px] transition-all hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)]">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                    <div className="p-6 sm:p-8 rounded-[1.5rem] bg-white/60 border border-white/80 backdrop-blur-xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] flex items-center gap-6 transition-all hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] hover:-translate-y-1">
                      <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-400 to-blue-600 text-white flex items-center justify-center shrink-0 shadow-lg shadow-blue-500/30">
                        <Wallet size={24} />
                      </div>
                      <div>
                        <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Required Investment</h3>
                        <p className="font-black text-slate-900 text-2xl tracking-tight">{idea.budget}</p>
                      </div>
                    </div>

                    <div className="p-6 sm:p-8 rounded-[1.5rem] bg-white/60 border border-white/80 backdrop-blur-xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] flex items-center gap-6 transition-all hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] hover:-translate-y-1">
                      <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-400 to-indigo-600 text-white flex items-center justify-center shrink-0 shadow-lg shadow-indigo-500/30">
                        <Users size={24} />
                      </div>
                      <div>
                        <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Resource Allocation</h3>
                  ...
                  <div className="w-full">
                    ...
                    <div className="p-6 sm:p-8 rounded-[1.5rem] border bg-amber-50/60 backdrop-blur-xl border-amber-200/80 shadow-[0_8px_30px_rgba(245,158,11,0.08)] flex gap-5 items-start transition-all hover:shadow-[0_8px_30px_rgba(245,158,11,0.15)]">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-amber-400 to-amber-500 flex items-center justify-center shrink-0 text-white shadow-lg shadow-amber-500/30">
                        <AlertTriangle size={20} />
                      </div>
                      <div className="pt-2 flex-1">
                        <p className="font-bold text-amber-950 text-[15px]">{idea.risks}</p>
                      </div>
                    </div>
                  </div>
              ...
              {/* Fixed Footer */}
              <div className="px-8 sm:px-12 py-6 bg-white/60 backdrop-blur-2xl border-t border-white/80 shrink-0 flex flex-wrap justify-end gap-4 relative z-20">
                <button
                  onClick={() => setReviewIdeaId(null)}
                  className="px-8 py-3.5 rounded-2xl text-[14px] font-extrabold bg-white/80 border border-slate-200/80 text-slate-600 hover:bg-white hover:shadow-md transition-all cursor-pointer backdrop-blur-md"
                >
                  Dismiss Review
                </button>
                {idea.status !== 'Approved' && (
                  <button
                    ...
                    className="px-8 py-3.5 rounded-2xl text-[14px] font-extrabold text-white shadow-xl shadow-indigo-500/40 hover:shadow-indigo-500/60 hover:-translate-y-1 transition-all cursor-pointer flex items-center gap-2 border border-white/20"
